    ...
    <>
      <nav className='bg-black'>
        <div className=' px-12 py-6 flex justify-end max-w-7xl mx-auto mb-12'>
          {userContext.user ? <button className='font-bold' onClick={signOut}>Sign Out</button> : <Link className='text-secondary font-bold' href={'/login'}>Sign In</Link>}
        </div>
      </nav>
      <main className="px-12 flex min-h-screen max-w-7xl mx-auto">
        <h1 className='font-bold text-5xl'>Hi, {'{user_name}'}</h1>
      </main>
    </>
  )
}
